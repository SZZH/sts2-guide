...
## 每日动作（按需触发）
- 按任务与信号变化触发巡检
- 当任务属于“数据调研/复盘/增长分析”时，覆盖 GSC + Vercel + Bing Webmaster + Google Trends
- 每日运维默认增加一次“近期游戏动态”巡检，默认看近窗增量而不是只看当天新闻；来源至少覆盖：官方公告、Steam 新闻、SteamDB、官方社媒
- 做游戏动态巡检前，先看 `docs/ops-center/08-game-dynamics-intel.md`；优先识别新增情报，不重复整理已记录条目
- 执行后更新相关文档（通常是 `01-daily-log.md` 与 `00-dashboard.md`）
- 外链状态检查与评论巡检（Reddit / X / Steam）在“有新内容/有积压/有异常”时优先执行
- 若游戏动态巡检命中新补丁、热修、平衡调整、官方公告或重要社媒信息，当轮必须补两个结论：`有哪些近期未关注增量`，以及 `是否发新闻页 / 是否补内链 / 是否提交 IndexNow`
- 若游戏动态巡检未命中新增事件，但补全了旧事件证据，也要更新情报台账中的原条目状态，不新建重复记录
...
- 硬闸门：识别出 `>=3` 个独立任务后，必须先分发再执行；若仍顺序执行，需先记录串行理由（依赖/锁/限速/用户要求）。
...
- [ ] 若本条社媒内容承接站内新页面或新改动，发帖前必须先确认：代码已提交并推送到 `main`、Vercel Production 部署为 `Ready`、正式域名目标 URL 返回 `HTTP 200` 且页面正文含目标内容。未完成任一项时禁止发帖。
...
- [ ] 建议统一主观测指标（6 项）：`总点击`、`总曝光`、`CTR`、`平均排名`、`非品牌词点击占比`、`四大查询页点击占比`
...
- [ ] 建议主攻词簇（英文）：`slay the spire 2 cards`、`slay the spire 2 relics`、`slay the spire 2 potions`、`slay the spire 2 characters`
- [ ] 建议高意图 news 词簇：`steam charts`、`steamdb`、`multiplayer/co-op`、`release date`
